...
void renderStartScreen(GameEngine* game) {
    ...
    game->renderText(renderer, bigFontVintage, "Poker", WINDOW_WIDTH / 2, 50, textColor, true);
    ...
    game->renderText(renderer, bigFontVintage, "PvP", PVP_BUTTON_X + BIG_BUTTON_WIDTH / 2, PVP_BUTTON_Y + BIG_BUTTON_HEIGHT / 2 - 45, textColor, true);
    ...
    game->renderText(renderer, bigFontVintage, "PvE", PVE_BUTTON_X + BIG_BUTTON_WIDTH / 2, PVE_BUTTON_Y + BIG_BUTTON_HEIGHT / 2 - 45, textColor, true);
...
void renderUsernames(GameEngine* game) {
    ...
    for (const std::string& username : usernames) {
        ...
        TTF_SizeText(smallFontVintage, username.c_str(), &textWidth, &textHeight);
        int deleteButtonWidth, deleteButtonHeight;
        TTF_SizeText(smallFontVintage, "x", &deleteButtonWidth, &deleteButtonHeight);
        int startX = WINDOW_WIDTH - textWidth - deleteButtonWidth - 30; // 10 pixels padding from the right and between username and 'x'
        
        // Render the username
        game->renderText(renderer, smallFontVintage, username.c_str(), startX, startY, textColor, false);

        // Render the 'x' button as text
        SDL_Rect deleteButtonRect = {startX + textWidth + 10, startY, deleteButtonWidth, deleteButtonHeight};
        game->renderText(renderer, smallFontVintage, "x", deleteButtonRect.x, deleteButtonRect.y, deleteColor, false);
